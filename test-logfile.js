...
const fs = require('fs');
const log = fs.createWriteStream('./msg-dump.log', {flags: 'w'});
...
const connect = () => {
    ...
    ws.on('message', (data) => {
        ...
        if (msg === '3') { return; } // ignore our pong responses
        write(`[${new Date().toISOString()}] MSG [${msg.length}b]: ${msg.substring(0, 800)}`);

        if (msg.startsWith('40')) {
            write('Socket.IO ready. Subscribing...');
...
setTimeout(() => { write('3min up, stopping.'); process.exit(0); }, 180000);
connect();
